    ...
    try {
      ...
      if (await file.exists()) {
        ...
        setState(() {
          ...
          _apiKeyCtrl.text = json['api_key'] ?? '';
          _temperature = (json['temperature'] as num?)?.toDouble() ?? 0.2;
          _maxTokensCtrl.text = (json['max_tokens'] ?? 2048).toString();
          _loading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> _testConnection() async {
    ...
    try {
      if (provider == 'ollama') {
        ...
        if (response.statusCode == 200) {
          ...
          try {
            ...
            setState(() => _connectionSuccess = 'Ollama baglanti basarili! ✓\nYuklu modeller: $modelList');
          } catch (_) {
            setState(() => _connectionSuccess = 'Ollama baglantisi basarili! ✓');
          }
        } else {
          setState(() => _connectionError = 'Ollama baglantisi basarisiz. HTTP ${response.statusCode}\nOllama calistigindan emin olun: ollama serve');
        }

      } else if (provider == 'gemini') {
        if (apiKey.isEmpty) throw Exception('API Anahtari bos birakilamaz');
        var geminiModel = model;
        if (!geminiModel.startsWith('models/') && !geminiModel.startsWith('gemini-')) {
        ...
        if (response.statusCode == 200) {
          setState(() => _connectionSuccess = 'Gemini baglantisi basarili! ✓\nModel: $geminiModel');
        } else if (response.statusCode == 400 || response.statusCode == 403) {
          final body = response.body;
          if (body.contains('API_KEY_INVALID') || body.contains('invalid') || body.contains('INVALID_ARGUMENT')) {
            setState(() => _connectionError = 'Gemini API Anahtari gecersiz.\nDogrulama: https://aistudio.google.com/app/apikey');
          } else {
            setState(() => _connectionError = 'Gemini hatasi (${response.statusCode}): $body');
          }
        } else {
          setState(() => _connectionError = 'Gemini yanit kodu: ${response.statusCode}\n${response.body.substring(0, response.body.length.clamp(0, 300))}');
        }
        ...
        if (modelsResp.statusCode == 200) {
          ...
          if (chatResp.statusCode == 200) {
            ...
            setState(() => _connectionError = 'OpenAI API Anahtari gecersiz (401).\nhttps://platform.openai.com/api-keys');
          } else {
            final body = chatResp.body;
            setState(() => _connectionError = 'OpenAI hatasi (${chatResp.statusCode}): ${body.substring(0, body.length.clamp(0, 400))}');
          }
        } else {
          setState(() => _connectionError = 'OpenAI yanit: ${modelsResp.statusCode}');
        }
        ...
        if (response.statusCode == 200) {
          setState(() => _connectionSuccess = 'Nvidia NIM baglantisi basarili! ✓\nModel: $model');
        } else if (response.statusCode == 401) {
          setState(() => _connectionError = 'Nvidia NIM API Anahtari gecersiz (401).\nhttps://build.nvidia.com/explore/reasoning');
        } else {
          setState(() => _connectionError = 'Nvidia NIM hatasi (${response.statusCode}): ${response.body.substring(0, response.body.length.clamp(0, 300))}');
        }
        ...
        if (response.statusCode == 200) {
          setState(() => _connectionSuccess = 'Claude (Anthropic) baglantisi basarili! ✓\nModel: $model');
        } else if (response.statusCode == 401) {
          setState(() => _connectionError = 'Claude API Anahtari gecersiz (401 Unauthorized).\nDogrulama: https://console.anthropic.com/settings/keys');
        } else if (response.statusCode == 400) {
          final body = response.body;
          if (body.contains('invalid_api_key')) {
            setState(() => _connectionError = 'Claude API Anahtari gecersiz.\nhttps://console.anthropic.com/settings/keys');
          } else if (body.contains('not_found_error') || body.contains('model')) {
            setState(() => _connectionError = 'Claude model bulunamadi: "$model"\nOrnek: claude-3-5-sonnet-20241022, claude-3-haiku-20240307');
          } else {
            setState(() => _connectionError = 'Claude 400 hatasi: ${body.substring(0, body.length.clamp(0, 400))}');
          }
        } else if (response.statusCode == 403) {
          setState(() => _connectionError = 'Claude erisim reddedildi (403).\nBu modele erisim izniniz olmayabilir.');
        } else {
          setState(() => _connectionError = 'Claude hatasi (${response.statusCode}): ${response.body.substring(0, response.body.length.clamp(0, 400))}');
        }
      }
      ...
      if (errMsg.contains('SocketException') || errMsg.contains('Connection refused')) {
        if (provider == 'ollama') {
          errMsg = 'Ollama servisi bulunamadi. "ollama serve" komutu ile baslatiniz.';
        } else {
          errMsg = 'Ag baglantisi kurulamadi. Internet baglantinizi kontrol edin.';
        }
      } else if (errMsg.contains('HandshakeException') || errMsg.contains('Certificate')) {
        errMsg = 'SSL/TLS hatasi. Sistem sertifikalari guncellenmis olmayabilir.';
      }
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      final settings = {
        ...
        'max_tokens': int.tryParse(_maxTokensCtrl.text.trim()) ?? 2048,
      };
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  ...
                  helperText: 'Ornek: gemma4, gemini-1.5-flash, gpt-4o, claude-3-5-sonnet-20241022, minimax/minimax-01',
                ),
                validator: (v) => v == null || v.trim().isEmpty
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: Text(_testingConnection ? 'Baglanti Test Ediliyor...' : 'Baglantiyi Test Et'),
                ),
              ),
              const SizedBox(height: 24),
    ...
    return Column(
      children: [
        ...
        _buildProviderRadio(
          ...
          description: 'Nvidia NIM OpenAI uyumlu bulut platformu (API Anahtari gerektirir).',
          icon: Icons.layers,
        ),
      ],
    );
      ...
      onTap: () {
        setState(() {
          ...
          if (id == 'ollama') {
            ...
            _modelCtrl.text = 'claude-3-5-sonnet-20241022';
            _baseUrlCtrl.text = '';
          } else if (id == 'nvidia') {
